...
//addphone
function addPhone() {
    const phoneName = nameProduct.value.trim();
    const phonePrice = price.value.trim();
    const phoneScreen = screen.value.trim();
    const phoneBackCam = backCamera.value.trim();
    const phoneFrontCam = frontCamera.value.trim();
    const phoneImgLink = image.value.trim();
    const phoneDes = description.value.trim();
...
function validation() {
    ...
    return (
        validate.reduce((total, current) => total && current, true) && isCheck
    );
}

const searchName = document.querySelector("#searchName");
searchName.addEventListener("input", () => {
    const filterSearch = listProduct.filter((product) =>
        product.name
            .toLowerCase()
...
selectPrice.addEventListener("change", () => {
    ...
    if (selectPrice.value === "asc") {
        const sortListProduct = listProduct.sort((a, b) => a.price - b.price);
        renderUI(sortListProduct);
    } else {
        const sortListProduct = listProduct.sort((a, b) => b.price - a.price);
        renderUI(sortListProduct);
    }
});
